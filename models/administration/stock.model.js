"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var stock_schema_1 = require("../../schemas/administration/stock.schema");
var base_model_1 = require("../base.model");
var enums_1 = require("../../utils/enums");
var StockModel = /** @class */ (function (_super) {
    __extends(StockModel, _super);
    function StockModel() {
        return _super.call(this, stock_schema_1.StockSchema, 'stock') || this;
    }
    StockModel.prototype.transfer = function (stock) {
        return __awaiter(this, void 0, void 0, function () {
            var stockModel, stocks, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        stockModel = new base_model_1.BaseModel(stock_schema_1.StockSchema, 'stock');
                        stocks = [{
                                item: stock.item,
                                quantity: stock.quantity,
                                type: 'in',
                                office: stock.office,
                                note: "Recibido desde la sucursal " + stock['source_office'].name + ".",
                                create_date: new Date(),
                                create_user: stock.create_user,
                                settings: stock['setting']
                            }, {
                                item: stock.item,
                                quantity: stock.quantity,
                                type: 'out',
                                office: stock['source_office'],
                                note: "Tranferido a la sucursal " + stock.office.name + ".",
                                create_date: new Date(),
                                create_user: stock.create_user,
                                settings: stock['setting']
                            }];
                        return [4 /*yield*/, stockModel.saveMeny(stocks)];
                    case 1: return [2 /*return*/, _a.sent()];
                    case 2:
                        error_1 = _a.sent();
                        console.log(error_1);
                        throw new Error("Error en la transferencia " + this.document_name);
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    StockModel.prototype.out_stock = function (invoice, note) {
        return __awaiter(this, void 0, void 0, function () {
            var stockModel, items, stocks;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        stockModel = new base_model_1.BaseModel(stock_schema_1.StockSchema, enums_1.COLLECTION_NAME_ENUM.stock), items = invoice.items.filter(function (item) {
                            return !item.service;
                        });
                        stocks = items.map(function (item) {
                            return {
                                item: item,
                                quantity: item.quantity,
                                type: enums_1.STOCK_TYPE_ENUM.out,
                                office: invoice.office,
                                note: note,
                                create_date: new Date(),
                                create_user: invoice.create_user,
                                setting: invoice.setting
                            };
                        });
                        if (!(stocks.length > 0)) return [3 /*break*/, 2];
                        return [4 /*yield*/, stockModel.saveMeny(stocks)];
                    case 1:
                        _a.sent();
                        _a.label = 2;
                    case 2: return [2 /*return*/];
                }
            });
        });
    };
    StockModel.prototype.in_stock = function (object, note) {
        return __awaiter(this, void 0, void 0, function () {
            var stockModel, items, stocks;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        stockModel = new base_model_1.BaseModel(stock_schema_1.StockSchema, enums_1.COLLECTION_NAME_ENUM.stock), items = object.items.filter(function (item) {
                            return !item.service;
                        });
                        stocks = items.map(function (item) {
                            return {
                                item: item,
                                quantity: item.quantity,
                                type: enums_1.STOCK_TYPE_ENUM.in,
                                office: object.office,
                                note: note,
                                create_date: new Date(),
                                create_user: object.create_user,
                                setting: object.setting
                            };
                        });
                        if (!(stocks.length > 0)) return [3 /*break*/, 2];
                        return [4 /*yield*/, stockModel.saveMeny(stocks)];
                    case 1:
                        _a.sent();
                        _a.label = 2;
                    case 2: return [2 /*return*/];
                }
            });
        });
    };
    return StockModel;
}(base_model_1.BaseModel));
exports.StockModel = StockModel;
//# sourceMappingURL=stock.model.js.map